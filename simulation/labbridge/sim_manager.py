...
MANAGER_WEB_DIR = Path(
    os.environ.get("LABBRIDGE_MANAGER_WEB_DIR", str(DEFAULT_SIM_ROOT / "web" / "manager"))
).resolve()
...
class SimManager:
    def __init__(
        ...
        *,
        sim_root: Path,
        runner: Path,
        manager_index_html: Path,
        manager_api_docs_html: Path,
    ):
        self.registry_path = registry_path
        self.sim_root = sim_root
        self.runner = runner
        self.manager_index_html = manager_index_html
        self.manager_api_docs_html = manager_api_docs_html
    ...
    def stop_pid(self, pid: int) -> dict[str, Any]:
        ...
        if pid not in scanned_pids:
            raise HTTPException(status_code=404, detail=f"pid={pid} is not a scanned sim process")
...
def create_app(manager: SimManager | None = None) -> FastAPI:
    mgr = manager or SimManager(
        REGISTRY_PATH,
        sim_root=DEFAULT_SIM_ROOT,
        runner=DEFAULT_RUNNER,
        manager_index_html=MANAGER_INDEX_HTML,
        manager_api_docs_html=MANAGER_API_DOCS_HTML,
    )

    app = FastAPI(title="LabBridge Sim Manager", version="1.0.0")

    @app.on_event("shutdown")
    def _shutdown_cleanup():
        try:
            result = mgr.stop_all_managed()
    ...
    @app.get("/")
    def manager_ui():
        if not mgr.manager_index_html.exists():
            raise HTTPException(status_code=500, detail=f"Missing UI file: {mgr.manager_index_html}")
        return FileResponse(str(mgr.manager_index_html))

    @app.get("/manager/docs")
    def manager_docs_page():
        if not mgr.manager_api_docs_html.exists():
            raise HTTPException(status_code=500, detail=f"Missing docs file: {mgr.manager_api_docs_html}")
        return FileResponse(str(mgr.manager_api_docs_html))
