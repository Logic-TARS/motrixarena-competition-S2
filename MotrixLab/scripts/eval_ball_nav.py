"""Deterministic evaluation of K1 ball-navigate policy across multiple episodes.

Usage:
    cd MotrixLab
    conda run -n sim_soccer_rl env PYTHONPATH=./motrix_envs/src:./motrix_rl/src \
        python scripts/eval_ball_nav.py --checkpoints runs/k1-ball-navigate/rslrl/<run>/model_299.pt
"""
...
def parse_args():
    ...
    parser.add_argument("--policy", default=None, help="Path to a single policy checkpoint")
...
def load_policy(checkpoint_path, device):
    ...
    class Policy(nn.Module):
        ...
        def forward(self, x):
            obs = (x - self.norm_mean) / (self.norm_std + 1e-8)
            return self.mlp(obs)
...
def evaluate(env, policy, device, num_envs, max_steps):
    ...
    ongoing_progress = current_start_dist - state.info["ball_dist"]
...
def main():
    ...
    env = registry.make("k1-ball-navigate", sim_backend="np", num_envs=args.num_envs)
    ...
    for ckpt in checkpoints:
        ...
        print(
            f"  Mean progress vel: {result['mean_progress_vel']:.4f} m/s  "
